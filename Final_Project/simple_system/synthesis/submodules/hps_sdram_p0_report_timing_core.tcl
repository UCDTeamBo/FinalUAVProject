...
proc hps_sdram_p0_perform_ac_analyses {opcs opcname inst scale_factors_name pin_array_name timing_parameters_array_name summary_name IP_name} {
	...
	foreach ck_pin $pins(ck_pins) {
		set leveling_delay_chain_name [hps_sdram_p0_traverse_to_leveling_delay_chain $ck_pin msg_list]
	...
	set clk_max [hps_sdram_p0_max_in_collection [get_path -rise_from ${pll_clock} -rise_to $clk_ldc_periphery_nodes] "arrival_time"]
	if {$clk_max <= 0} {set clk_max -9999}
	set clk_min [hps_sdram_p0_max_in_collection [get_path -rise_from ${pll_clock} -rise_to $clk_ldc_periphery_nodes -min_path] "arrival_time"]
	...
	foreach ac_pin $ac_pins {
		...
		set leveling_delay_chain_name [hps_sdram_p0_traverse_to_leveling_delay_chain $ac_pin msg_list]
		...
		set ac_max  [hps_sdram_p0_max_in_collection [get_path -rise_from ${pll_clock} -rise_to $ac_ldc_periphery_node] "arrival_time"]
		if {$ac_max <= 0} {set ac_max -9999}
		set ac_min  [hps_sdram_p0_max_in_collection [get_path -rise_from ${pll_clock} -rise_to $ac_ldc_periphery_node -min_path] "arrival_time"]
		if {$ac_min <= 0} {set ac_min 9999}
		set extra_ccpr [hps_sdram_p0_round_3dp [expr {$clk_max - $clk_min + $ac_max - $ac_min}]]
